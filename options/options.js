    ...
    chrome.storage.sync.get({
        ...
        if (webhookDataArray !== '') {
            ...
            Object.keys(webhookDataArray).forEach(key => {
                ...
                if (object.webhookAddressInput !== '' || object.whitelistInput !== '' || object.blacklistInput !== '') {
                    inUseString = ' (live)';
                }
...
function loadHTMLAll() {
    ...
    function createExtraWebpage(currentObject, currentNavScroll) {
        ...
        if (currentObject.id === 1000) {
            ...
            document.getElementById(section).appendChild(document.createElement('hr'));
            ...
            createLabel("Step 3: Click on the SETTINGS for a Discord Text Channel (the GEAR icon)");
            // insertImage(6);
            ...
            createLabel("Step 7: Right-click the Grobel Extension icon and click on OPTIONS");
            insertImage(12);

            createLabel("Step 8: Use CTRL-V to paste the link into the WEBHOOK ADDRESS");
            insertImage(13);

            createLabel("NOTE: Settings are automatically saved");
            insertImage(14);

            createLabel("Set-up is complete! Try using the extension on other websites.");
            insertImage(15);
        ...
        if (currentObject.id === 1001) {
            ...
            createLabelWithNewline("NOTE: Be careful making this number smaller. Webhooks in excess will be erased!");
            ...
            function createWebhookRadioArray() {
                ...
                Array(26).fill().map((x, i) => i).forEach(int => {
                    if(int > 0) {
        ...
        function createGettingStartedEmbed() {
            ...
            div.innerHTML = '<iframe width="560" height="315" src="https://www.youtube.com/embed/memQMYknbpY" frameborder="0" allow="accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>';
    ...
    function createWebhookWebpage(currentObject, currentNavScroll) {
        ...
        if (num === 0) {
            ...
            globalSettingsInfoLabel.innerHTML = "NOTE: These settings will apply to ALL webhooks.";
    ...
    function createRequiredSettingsFormV2(currentObject) {
        ...
        input.setAttribute("placeholder", "https://discordapp.com/api/webhooks/...");
    ...
    function createFilteringSettingsFormV2(currentObject) {
        ...
        radioLabel2.innerHTML = " Whitelist: Websites will be allowed ONLY IF the URL contains these terms.";
        ...
        radioLabel3.innerHTML = " Blacklist: Websites will be allowed UNLESS the URL contains these terms.";
        ...
        whitelistInput.setAttribute("placeholder", "Example: 'reddit nytimes arstechnica'");
        ...
        blacklistInput.setAttribute("placeholder", "Example: 'mail drive braisers'");
    ...
    function createAutoGrobelSettingsFormV2(currentObject) {
        ...
        var label = document.createElement('label');
        label.innerHTML = "Description: Auto-Grobel will automatically save webpages whenever a page is loaded.";
        ...
        noteLabel.innerHTML = "(Note: Auto-Grobel follows all Whitelist/Blacklist rules.)";
        var noteLabel2 = document.createElement('label');
        noteLabel2.innerHTML = "(Note 2: Auto-Grobel will disable the Manual-Button-Press Grobel for this webhook.)";
...
function webhookObjectArrayInit() {
    ...
    Array(20).fill().map((x, i) => i).forEach(notNum => {
        let num = notNum + 1;
...
function extraNavigationInit() {
    ...
    extraNavigationObjects[1001] = createExtraNavigationObject(1001, "Premium Settings");
...
function autosave(currentObject) {
    ...
    chrome.storage.sync.get({
        ...
        if (items.currentPage !== -1 && items.webhookDataArray !== '') {
            ...
            // NavBar Stuff
            if (newObject.webhookAddressInput !== '' || newObject.whitelistInput !== '' || newObject.blacklistInput !== '') {
...
function renderDocument() {
    ...
    function renderFilteringSettings() {
        ...
        if (filteringOptionsRadio === "None") {
            ...
            document.getElementById("whitelistRadioLabel").setAttribute("class", "disabled");
            document.getElementById("blacklistRadioLabel").setAttribute("class", "disabled");
        } else if (filteringOptionsRadio === "Whitelist") {
            document.getElementById("noneRadioLabel").setAttribute("class", "disabled");
            document.getElementById("whitelistRadioLabel").classList.remove("disabled");
            document.getElementById("blacklistRadioLabel").setAttribute("class", "disabled");
        } else if (filteringOptionsRadio === "Blacklist") {
            document.getElementById("noneRadioLabel").setAttribute("class", "disabled");
            document.getElementById("whitelistRadioLabel").setAttribute("class", "disabled");
            document.getElementById("blacklistRadioLabel").classList.remove("disabled");
        }
...
function compareSave(oldObject, newObject) {
    ...
    if (Math.abs(oldObject["whitelistInput"].length - newObject["whitelistInput"].length) === 1) {

        document.getElementById("filterRadioWhitelist").checked = true;

        return "Whitelist";
    } else if (Math.abs(oldObject["blacklistInput"].length - newObject["blacklistInput"].length) === 1) {
